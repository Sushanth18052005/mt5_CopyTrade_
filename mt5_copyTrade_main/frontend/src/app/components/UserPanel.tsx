...
export default function UserPanel({ onLogout }: UserPanelProps) {
  ...
  const fetchEAStatus = async () => {
    try {
      ...
      const response = await fetch('https://mt5-copytrade.onrender.com/api/v1/group/trading-controls', {
        headers: {
  ...
  const toggleEAStatus = async () => {
    ...
    try {
      ...
      const response = await fetch('https://mt5-copytrade.onrender.com/api/v1/group/trading-controls/toggle', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const API_URL = 'https://mt5-copytrade.onrender.com';
    ...
    (async () => {
      try {
        const token = localStorage.getItem('authToken');
        const API_URL = 'https://mt5-copytrade.onrender.com';
        const res = await fetch(`${API_URL}/api/v1/groups/list?status=active`, {
          method: 'GET',
          headers: token ? { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } : { 'Content-Type': 'application/json' }
        });
        if (res.ok) {
          const d = await res.json();
          if (d.success && d.data && d.data.groups) setAvailableGroups(d.data.groups);
        }
      } catch (err) {
  ...
  const joinByApiKey = async (apiKey?: string) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        setApiKeyInput('');
        // Refresh groups list
        const r2 = await fetch(`${API_URL}/api/v1/groups/list?status=active`);
        if (r2.ok) {
          const d2 = await r2.json();
          if (d2.success && d2.data && d2.data.groups) setAvailableGroups(d2.data.groups);
        }
      } else {
      ...
      <div className="card" style={{ marginTop: '20px' }}>
        ...
        <div style={{ padding: '20px' }}>
          {dashboardData?.user_info.group_name ? (
            <div style={{ color: '#6b7280' }}>You are currently in <strong>{dashboardData.user_info.group_name}</strong></div>
          ) : (
            <>
              <div style={{ marginBottom: '12px' }}>
                {availableGroups.length === 0 ? (
                  <div style={{ color: '#6b7280' }}>No groups available right now.</div>
                ) : (
                  <div>
                    {availableGroups.map((g: any) => (
                      <div key={g._id || g.id || g.group_name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #f3f4f6' }}>
                        <div>
                          <div style={{ fontWeight: 'bold' }}>{g.group_name}</div>
                          <div style={{ fontSize: '12px', color: '#6b7280' }}>{g.description || g.group_type || ''}</div>
                        </div>
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                          <button className="btn btn-outline btn-sm" onClick={() => joinByApiKey(g.api_key)} disabled={joining}>{joining ? 'Joining...' : 'Join'}</button>
                          <div style={{ fontSize: '12px', color: '#6b7280' }}>API: {g.api_key ? `${g.api_key.slice(0, 6)}...` : '—'}</div>
                        </div>
                      </div>
                    ))}
                  </div>
